...
func abs(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value
	...
	case Int:
		...
		if err := thread.DeclareSizeIncrease(x.Size(), b.Name()); err != nil {
			return nil, err
		}
...
func bool_(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value = False
...
func bytes_(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
	...
	case String:
		...
		if err := thread.DeclareSizeIncrease(1+uintptr(len(x)), b.Name()); err != nil {
			return nil, err
		}
	...
	case Iterable:
		...
		if err := thread.DeclareSizeIncrease(1+uintptr(IterableLen(x)), b.Name()); err != nil {
			return nil, err
		}
		iter := x.Iterate()
		defer iter.Done()
...
func chr(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
...
func dir(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
	...
	if err := thread.DeclareSizeIncrease(1+uintptr(len(names)), b.Name()); err != nil {
		return nil, err
	}
	sort.Strings(names)
...
func enumerate(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var iterable Iterable
	var start int
	...
	if err := thread.DeclareSizeIncrease(1+uintptr(IterableLen(iterable)), b.Name()); err != nil {
		return nil, err
	}
...
func hash(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value
...
func int_(thread *Thread, b_ *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value = zero
	var base Value
...
func len_(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value
...
func list(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var iterable Iterable
	...
	if iterable != nil {
		if err := thread.DeclareSizeIncrease(uintptr(IterableLen(iterable)), b.Name()); err != nil {
			return nil, err
		}
		iter := iterable.Iterate()
		defer iter.Done()
...
func ord(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
...
func repr(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var x Value
	if err := UnpackPositionalArgs("repr", args, kwargs, 1, &x); err != nil {
		return nil, err
	}
	xs := x.String()
	if err := thread.DeclareSizeIncrease(1+uintptr(len(xs)), b.Name()); err != nil {
		return nil, err
	}
...
func reversed(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var iterable Iterable
...
func set(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	if iterable != nil {
		if err := thread.DeclareSizeIncrease(1+uintptr(IterableLen(iterable)), b.Name()); err != nil {
			// Assumes list contains only unique elements
			return nil, err
		}
		iter := iterable.Iterate()
		defer iter.Done()
...
func sorted(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	if err := thread.DeclareSizeIncrease(2+uintptr(IterableLen(iterable)), b.Name()); err != nil {
		return nil, err
	}
	...
	var keys []Value
	if key != nil {
		if err := thread.DeclareSizeIncrease(1+uintptr(len(values)), b.Name()); err != nil {
			return nil, err
		}
...
func str(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
	...
	case String:
		return x, nil
	case Bytes:
		if err := thread.DeclareSizeIncrease(uintptr(len(string(x))), b.Name()); err != nil {
			return nil, err
		}
		// Invalid encodings are replaced by that of U+FFFD.
	...
	default:
		xs := x.String()
		if err := thread.DeclareSizeIncrease(uintptr(len(xs)), b.Name()); err != nil {
			return nil, err
		}
		return String(xs), nil
	}
...
func tuple(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var iterable Iterable
...
func zip(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if len(kwargs) > 0 {
	...
	if rows >= 0 {
		// length known
		if err := thread.DeclareSizeIncrease(3+uintptr((1+cols)*rows), b.Name()); err != nil {
			return nil, err
		}
	...
	outer:
		for {
			if err := thread.DeclareSizeIncrease(uintptr(2+cols), b.Name()); err != nil {
				return nil, err
			}
...
func dict_clear(_ *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	// TODO(kcza): track location usage, negative delta
...
func dict_items(thread *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	...
	if err := thread.DeclareSizeIncrease(1+uintptr(dict.Len()), b.Name()); err != nil {
		return nil, err
	}
	items := dict.Items()
...
func list_extend(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := b.Receiver().(*List)
	var iterable Iterable
	...
	if err := th.DeclareSizeIncrease(uintptr(IterableLen(iterable)), b.Name()); err != nil {
		return nil, err
	}
...
func list_insert(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := b.Receiver().(*List)
	var index int
	var object Value
	...
	if err := th.DeclareSizeIncrease(1, b.Name()); err != nil {
		return nil, err
	}
...
func string_capitalize(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0); err != nil {
		return nil, err
	}
	s := string(b.Receiver().(String))
	if err := th.DeclareSizeIncrease(1+uintptr(len(s)), b.Name()); err != nil {
		return nil, err
	}
...
func string_format(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	format := string(b.Receiver().(String))
	var auto, manual bool // kinds of positional indexing used
	if err := th.DeclareSizeIncrease(2+uintptr(len(format)), b.Name()); err != nil {
		return nil, err
	}
	buf := new(strings.Builder)
	index := 0
	for {
		...
		field := format[:i]
		format = format[i+1:]
		th.DeclareSizeDecrease(uintptr(len("{}") + len(field)))
...
func string_join(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := string(b.Receiver().(String))
	var iterable Iterable
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 1, &iterable); err != nil {
		return nil, err
	}
	if err := th.DeclareSizeIncrease(uintptr(len(recv)*(IterableLen(iterable)-1)), b.Name()); err != nil {
		return nil, err
	}
	iter := iterable.Iterate()
	defer iter.Done()
	...
	for i := 0; iter.Next(&x); i++ {
		...
		if err := th.DeclareSizeIncrease(uintptr(len(s)), b.Name()); err != nil {
			return nil, err
		}
		buf.WriteString(s)
	}
...
func string_lower(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0); err != nil {
		return nil, err
	}
	recv := b.Receiver().(String)
	if err := th.DeclareSizeIncrease(uintptr(recv.Len()), b.Name()); err != nil {
		return nil, err
	}
	return String(strings.ToLower(string(recv))), nil
}

// https://github.com/google/starlark-go/blob/master/doc/spec.md#string·partition
func string_partition(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := string(b.Receiver().(String))
	var sep string
	...
	if err := th.DeclareSizeIncrease(4+(3+uintptr(len(recv))), b.Name()); err != nil {
		return nil, err
	}
	var i int
	if b.Name()[0] == 'p' {
...
func string_removefix(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := string(b.Receiver().(String))
	var fix string
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 1, &fix); err != nil {
		return nil, err
	}
	if err := th.DeclareSizeIncrease(uintptr(len(recv)), b.Name()); err != nil {
		// Assume worst-case—where fix is absent
		return nil, err
	}
...
func string_replace(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := string(b.Receiver().(String))
	var old, new string
	count := -1
	...
	additionalAllocationsEstimate := uintptr(len(recv))
	if count > 0 {
		if extra := (len(new) - len(old)) * count; extra > 0 {
			additionalAllocationsEstimate += uintptr(extra)
		}
	}
	if err := th.DeclareSizeIncrease(uintptr(additionalAllocationsEstimate), b.Name()); err != nil {
		return nil, err
	}
	str := strings.Replace(recv, old, new, count)
	if difference := uintptr(1+len(str)) - additionalAllocationsEstimate; difference > 0 {
		if err := th.DeclareSizeIncrease(difference, b.Name()); err != nil {
			return nil, err
		}
	} else {
		th.DeclareSizeDecrease(difference)
	}
...
func string_strip(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var chars string
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0, &chars); err != nil {
		return nil, err
	}
	recv := string(b.Receiver().(String))
	returnSizeEstimate := len(recv)
	if err := th.DeclareSizeIncrease(1+uintptr(returnSizeEstimate), b.Name()); err != nil {
		return nil, err
	}
	var s string
	switch b.Name()[0] {
	...
	if len(s) < returnSizeEstimate {
		th.DeclareSizeDecrease(uintptr(returnSizeEstimate - len(s)))
	}
	return String(s), nil
}

// https://github.com/google/starlark-go/blob/master/doc/spec.md#string·title
func string_title(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0); err != nil {
		return nil, err
	}
...
func string_upper(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0); err != nil {
		return nil, err
	}
	recv := string(b.Receiver().(String))
...
func string_split(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	recv := string(b.Receiver().(String))
	var sep_ Value
	maxsplit := -1
	...
	maxAllocations := 1 + len(recv)
	var splitAllocations int
	if maxsplit >= 0 {
		splitAllocations = 2 * maxsplit
	} else {
		splitAllocations = len(recv) / 2
	}
	maxAllocations += splitAllocations
	if err := th.DeclareSizeIncrease(uintptr(maxAllocations), b.Name()); err != nil {
		return nil, err
	}
	...
	if sep_ == nil || sep_ == None {
		...
		if maxsplit < 0 {
			...
			var err error
			res, err = splitspace(recv, maxsplit)
			if err != nil {
				return nil, err
			}
		} else { // rsplit
			var err error
			res, err = rsplitspace(recv, maxsplit)
			if err != nil {
				return nil, err
			}
		}
	...
	if len(res) < splitAllocations {
		th.DeclareSizeDecrease(uintptr(splitAllocations - len(res)))
	}
...
// Precondition: max >= 0.
func rsplitspace(s string, max int) ([]string, error) {
...
// Precondition: max >= 0.
func splitspace(s string, max int) ([]string, error) {
	var res []string
...
func string_splitlines(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var keepends bool
	...
	if err := th.DeclareSizeIncrease(1+uintptr(len(lines)), b.Name()); err != nil {
		return nil, err
	}
...
func set_union(th *Thread, b *Builtin, args Tuple, kwargs []Tuple) (Value, error) {
	var iterable Iterable
	if err := UnpackPositionalArgs(b.Name(), args, kwargs, 0, &iterable); err != nil {
		return nil, err
	}
	recv := b.Receiver().(*Set)
	if err := th.DeclareSizeIncrease(uintptr(1+recv.Len()+IterableLen(iterable)), b.Name()); err != nil {
		return nil, err
	}
	iter := iterable.Iterate()
	defer iter.Done()
	union, err := recv.Union(iter)
	if err != nil {
		return nil, nameErr(b, err)
	}
	return union, nil
